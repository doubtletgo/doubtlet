...
import { FindCosineData } from '../../utils/constants/Angle-table';
...
const Cosine = () => {
  ...
  useEffect(() => {
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the value of Cos ({${parseNumber(p || 'R')}${
              usePI ? '\\pi' : ''
  ...
  useEffect(() => {
    ...
    const staticValues = FindCosineData(angle);
    ...
    const stepOfDegree = [
      ...
      },
      {
        value: `Since ${parseNumber(nDivide90)} multiple is an <b>${
          isEven ? 'Even' : 'Odd'
        }</b> number so <b>Cos</b> will ${
          isEven ? 'remain <b>Cos</b>' : 'become <b>Sine</b>'
        }.
      `,
        type: 'span',
      },
      'br',
      {
        value: `Given angle lies in the ${quadrant}<sup>th</sup> quadrant so the value of Cosine will be <b>${
    ...
    const steps = [
      ...
      {
        value: putSpace(
          `The Cosine of an angle is defined as the ratio of the length of the `
        ),
        type: 'equation',
      },
      {
        value: putSpace(`Base to the Hypotenuse.`),
        type: 'equation',
      },
      {
        value: putSpace(
          `The Cosine value will be \\bold{positive} if it lies in the \\bold{1^st}or  \\bold{4^th} quadrant`
        ),
        type: 'equation',
      },
      {
        value: putSpace(`Cos = {Base\\above{1pt}Hypotenuse}`),
        type: 'equation',
      },
      'hr',
      {
...

      {
        value: isNegative
          ? `Since Cos is an <b>
              even function</b> so “-” negative sign will not come out.`
          : '',
        type: 'span',
      },
      'br',
      {
        value: `Cos(${parseNumber(angle, {}, 4)})=${
          isNegative ? '' : '-'
        } Cos(${parseNumber(abs(angle), {}, 4)})  = ${
          isNegative ? '' : '-'
        } Cos{(90 x ${parseNumber(nDivide90, {}, 4)} ) + ${parseNumber(
          nDivide90Remainder,
          {},
          4
        )})} = <b> ${isNegative || !neg ? '' : '-'}${
      ...
      'br',
      {
        value: `Now \\space value \\space of \\space ${Cos < 0 ? '-' : ''}${
          isEven ? 'Cos' : 'Sin'
        }(${parseNumber(nDivide90Remainder, {}, 4)}) = ${
          staticValues ? `${staticValues}` : `${parseNumber(Cos, {}, 2)}`
        }`,
        type: 'equation',
      },
      'hr',
