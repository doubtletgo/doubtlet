...
const NCR = () => {
  const [p, setP] = useLocalStorage('Ncr_p', '10');
  const [q, setQ] = useLocalStorage('Ncr_q', '2');
  const [equation, setEquation] = useLocalStorage('Ncr_equation', '');
  const [solution, setSolution] = useLocalStorage('Ncr_solution', '');
  const [result, setResult] = useLocalStorage('Ncr_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('Ncr_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('Ncr_showSteps', true);
  const [note, setNote] = useLocalStorage('Ncr_note', undefined);
  ...
  useEffect(() => {
    ...
    setEquation(
      renderSteps([
        ...
        'br',
        {
          value: `Value of r: - < ${parseNumber(q || 'r')} >  `,
          type: 'span',
        },
        'br',
        {
          value: `<sup>n</sup>C<sub>r</sub> : - < <sup>${parseNumber(
            p || 'n'
          )}</sup>C<sub>${parseNumber(q || 'r')}</sub> >  `,
          type: 'span',
        },
    ...
    const DefaultSolution = () => {
      ...
      const finalAnswer = [
        {
          value: `The value of <sup>${parseNumber(p)}</sup>C<sub>${parseNumber(
            q
          )}</sub> is <b>${ncr_calculation(
            parseNumber(p),
            parseNumber(q)
          )}</b>`,
          type: 'span',
      ...
      const steps = [
        ...
        ,
        {
          value: `The \\space value \\space of \\dbinom n r = {(n!) \\above{1pt} (r!) (n-r)!}`,
          type: 'equation',
        },
        'br',
        {
        ...
        'br',
        {
          ...
          = {${parseNumber(p)}!\\above{1pt}(${parseNumber(q)}!)  (${
            p + '-' + q
          })!}
          = {${parseNumber(p)}!\\above{1pt}(${parseNumber(q)}!)  (${p - q})!}
          = {${factorial(p)}\\above{1pt}(${factorial(q)})  (${factorial(
            p - q
          )})}
          \\space \\space = ${ncr_calculation(p, q)}`,
          type: 'equation',
        },
        'br',
        {
