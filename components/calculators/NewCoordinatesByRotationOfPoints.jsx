...
const NewCoordinatesByRotationOfPoints = () => {
  ...
  const tempX = convertFromLatex(x);
  const tempY = convertFromLatex(y);
    ...
    setNote(
      renderSteps([
        ...
        },
        {
          value: putSpace(
            `
            Calculate the coordinates of the new point obtained after rotating the`
          ),
          type: 'equation',
        },
        {
          value: putSpace(
            `given point(x, y)=\\bigg(${x || 'x'},${
              y || 'y'
            }\\bigg) by an angle θ=\\bold{ ${isClokwise ? '-' : ''}{${
              q == 1
                ? p || '\\theta'
  ...
  useEffect(() => {
    ...
    const cosDeg = parseNumber(
      Math.cos(isClokwise ? -valueToCalculate : valueToCalculate),
      {},
      3
    );
    const sinDeg = parseNumber(
      Math.sin(isClokwise ? -valueToCalculate : valueToCalculate),
      {},
      5
    );
    ...
    var subtraction = evalLatex(
      `${resultCosX}${minusSymbol(resultSinY)}${abs(resultSinY)}`
    )?.toString();
    var value1 = valueX * cosDeg - valueY * sinDeg;
    var value2 = valueX * sinDeg + valueY * cosDeg;
    var addition = evalLatex(
    ...
    const finalAnswer = [
      {
        value: removeSpace(` 
        The coordinates of the new point obtained after rotating the given point (x, y) `),
        type: 'span',
      },

      {
        value: putSpace(
          `=\\bigg({${convertIntoLatex(xValue)}},{${convertIntoLatex(
            yValue
          )}}\\bigg) by an angle θ = 30 degrees in ${
      ...
      },
      {
        ...
        \\bigg)  or 
        \\bigg({${parseNumber(subtraction, {}, 2)}} , {${parseNumber(
                addition,
                {},
                2
              )}}`
    ...
    const steps = [
      ...
      {
        value: putSpace(`x= {${convertIntoLatex(xValue)}}`),
        type: 'equation',
      },
      {
        value: `y ={${convertIntoLatex(yValue)}}`,
        type: 'equation',
      },
      {
        value: putSpace(
          ...
          }} ${usePI ? '\\pi' : ''} ${isDegree ? 'Degree' : 'radian'}} ${
            isDegree
              ? '\\degree'
              : `= ${isClokwise ? '-' : ''}${
                  usePI
                    ? `${parseNumber(piIntoR, {}, 3)}\\degree`
                    : `${parseNumber(finalResult, {}, 3)} radian`
                }`
          }`
        ),
        type: 'equation',
      ...
      },
      {
        value: putSpace(
          isDegree
            ? `x^,={${convertIntoLatex(xValue)}} cos({${
                isClokwise ? '-' : ''
              }}{${
                denumerator == 1
                  ? numerator
                  : [numerator, denumerator].join('\\above{1pt}')
              }}\\degree) - {${convertIntoLatex(yValue)}} sin({${
                isClokwise ? '-' : ''
              }}{${
                denumerator == 1
                  ? numerator
            ...
            : `x^,={${convertIntoLatex(xValue)}} cos({${
              ...
              }} ${usePI ? '\\degree' : 'rad'})  - {${convertIntoLatex(
                yValue
              )}} sin({${
                usePI
                  ? parseNumber(piIntoR, {}, 3)
      ...
      },
      {
        value: removeSpace(
          putSpace(
            `{${
              isValue
                ? `=({${convertIntoLatex(xValue)}})({${parseNumber(
                    cosR,
                    {},
                    3
                  )}})-({${convertIntoLatex(yValue)}})({${parseNumber(
                    sinR,
                    {},
                    3
                  )}}) ={${cosAnsX}} ${minusSymbol(resultSinY)}{${removeSymbol(
                    sinAnsY
                  )}}= {${parseNumber(resultCosX, {}, 5)}}{${minusSymbol(
                    resultSinY
                ...
                : `=({${convertIntoLatex(xValue)}})({${parseNumber(
                    ...
                    {},
                    3
                  )}}) = {${parseNumber(valueX * cosDeg, {}, 3)}}{${minusSymbol(
                    valueY * sinDeg
                  )}}  {${abs(
      ...
      {
        value: `<a href = "/calculator/sine/?a=${parseNumber(
          isClokwise ? -1 * valueToCalculate : valueToCalculate,
          {},
          3
        )}" target="_blank">to see the steps for sine calculation, click here</a>`,
        type: 'span',
        className: 'text-decoration-underline',
      },
      'br',
      {
        value: removeSpace(
          putSpace(
            isDegree
              ? `y^,= {${convertIntoLatex(xValue)}}   sin({${
                  isClokwise ? '-' : ''
                }}{${
                  denumerator == 1
                    ? numerator
                    : [numerator, denumerator].join('\\above{1pt}')
                }}\\degree)+ {${convertIntoLatex(yValue)}} cos({${
                  isClokwise ? '-' : ''
                }}{${
                  denumerator == 1
                    ? numerator
        ...
        value: putSpace(
          `{${
            isValue
              ? `=({${convertIntoLatex(
                  xValue
                )}})({${sinR}})+({${convertIntoLatex(
                  yValue
                )}})({${cosR}}) = {${sinAnsX}} ${addSymbol(
                  resultCosY
                  ...
                  {},
                  2
                )}} =  {${parseNumber(addition, {}, 2)}}`
              : `=({${convertIntoLatex(
                  xValue
                )}})({${sinDeg}}) + ({${convertIntoLatex(
                  yValue
                )}})({${parseNumber(cosDeg, {}, 2)}}) ={${parseNumber(
                  valueX * sinDeg
                )}} {${addSymbol(valueY * cosDeg)}} {${abs(
      ...
      {
        value: `<a href = "/calculator/cosine/?a=${parseNumber(
          isClokwise ? -1 * valueToCalculate : valueToCalculate,
          {},
          3
...
export default NewCoordinatesByRotationOfPoints;
