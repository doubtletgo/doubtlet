...
const CotInverse = () => {
  const [latex, setLatex] = useLocalStorage('CotInverse_latex', '-1');
  const [n, setN] = useLocalStorage('CotInverse_n', '');
  const [equation, setEquation] = useLocalStorage('CotInverse_equation', '');
  const [solution, setSolution] = useLocalStorage('CotInverse_solution', '');
  const [result, setResult] = useLocalStorage('CotInverse_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('CotInverse_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('CotInverse_showSteps', true);
  const [note, setNote] = useLocalStorage('CotInverse_note', undefined);
  const [degree, setDegree] = useLocalStorage('CotInverse_degree', 'Degree');
  const [input, setInput] = useLocalStorage('CotInverse_input', undefined);
  ...
  useEffect(() => {
    ...
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the value of cot^{-1} {${latex
              .replace('\\frac', '')
              .split('}{')
              .join('\\above{1pt}')}}`
          ),
  ...
  useEffect(() => {
    ...
    const radian = parseNumber(Math.atan(1 / input), {}, 3);
    const radian2 = parseNumber(
      ((180 - abs(radToDeg(radian))) * 3.14) / 180,
      {},
      3
    );
    const degToRad2 = fraction(radToDeg(radian), 180);
    const degToRad = getSubtraction(
      [1, degToRad2[0]],
      [1, degToRad2[1]],
      false
    );
    ...
    const finalAnswer = [
      {
        value: putSpace(
          `The principal value of cot^{-1}({${valueToShow}}) is \\bold{${
            isNeg ? 180 - abs(radToDeg(radian)) : radToDeg(radian)
          }^{o}} or \\bold{${p == 0 ? '0' : valueWithPI}} or \\bold{${
            isNeg ? radian2 : radian
          }}.`
        ),
        type: `equation`,
      },
      {
        value: putSpace(
          `\\& General Solution is cot^{-1}({${valueToShow}}) = \\bold{n\\pi + ({${
            p == 0 ? '0' : valueWithPI
          }})}`
        ),
        type: 'equation',
    ...
    const steps = [
      ...
      'br',
      {
        value: `The inverse of the cot of the given value is represented as 
        `,
        type: 'span',
      },
      {
        value: putSpace(
          `y = cot^{-1}(x) or y = arccot(x) or y = acot(x) \\& defined by a function such that x = cot(y)`
        ),
        type: 'equation',
      },
      {
        value: `The domain of the cot inverse x is (-∞, ∞).`,
        type: 'span',
      },
      {
        value: putSpace(`The range of the function is (0, \\pi).`),
        type: 'equation',
      },
      {
      ...
      {
        value: putSpace(
          `Principal Solution:- Which will lie under the range of the function i.e. (0, \\pi). `
        ),
        type: 'equation',
      },
      {
      ...
      {
        value: `\\bold{y = (n\\pi + cot^{-1}(x)) }`,
        type: 'equation',
      },
      {
        value: `where n is an integer & cot<sup>-1</sup>(x) represents the principal values of the function only.`,
        type: 'span',
      },
      'br',
      {
      ...
      },
      {
        value: isNeg
          ? putSpace(
              `Using the following property: cot^{-1}(-x) = \\pi -cot^{-1}(x)`
            )
          : '',
      ...
      {
        value: putSpace(`
          ${isNeg ? `cot^{-1}({${valueToShow})} =` : ' '} ${
          isNeg ? `\\pi -` : ''
        }cot^{-1}({${removeSymbol(valueToShow)}}) =  ${
          isNeg ? `\\pi - {${valueWithPI2}} =` : ''
        }({${valueWithPI}}) radian or ${
          isNeg ? 180 - abs(radToDeg(radian)) : radToDeg(radian)
        } degree`),
        type: 'equation',
      },
      ...
      {
        value: `
        From the table of the cot values, we can determine the angle such that`,
        type: 'span',
      },
      {
        value: putSpace(
          `Principal Solution is cot^{-1}({${valueToShow}}) = ({${
            p == 0 ? '0' : valueWithPI
          }}) radian or ${
            isNeg ? 180 - abs(radToDeg(radian)) : radToDeg(radian)
          } degrees`
      ...
      {
        value: putSpace(
          `General Solution is cot^{-1}({${valueToShow}}) = n\\pi + ({${
            p == 0 ? '0' : valueWithPI
          }})`
        ),
        type: 'equation',
  ...
  const hasValue = [input].some((v) => !!v && !isNaN(v));
