...
const HyperbolicSine = () => {
  const [latex, setLatex] = useLocalStorage('HyperbolicSine_latex', '3');
  const [a, setA] = useLocalStorage('HyperbolicSine_a', '');
  const [equation, setEquation] = useLocalStorage('HyperbolicSine_equation', '');
  const [solution, setSolution] = useLocalStorage('HyperbolicSine_solution', '');
  const [result, setResult] = useLocalStorage('HyperbolicSine_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('HyperbolicSine_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('HyperbolicSine_showSteps', true);
  const [note, setNote] = useLocalStorage('HyperbolicSine_note', undefined);
  const [input, setInput] = useLocalStorage('HyperbolicSine_input', undefined);
  ...
  useEffect(() => {
    ...
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the value of Sinh( {${latex
              .replace('\\frac', '')
              .split('}{')
              .join('\\above{1pt}')}})`
          ),
  ...
  useEffect(() => {
    ...
    const finalAnswer = [
      {
        value: `The \\space value \\space of \\space Sinh({${parseNumber(
          p || '(X)'
        )} ${
    ...
    const steps = [
      ...
      'br',
      {
        value: `The term <b>Sinh(X)</b> represents the hyperbolic Sine function.It is a mathematical function <br/> that 
        ...
        The hyperbolic Sine function Sinh x, can be defined using exponential functions as follows:
        `,
        type: 'span',
      },
      'br',
      {
        value: `Sinh(X) = \\bold{\\frac{(e^X-e^{-X})}{2}}`,
        type: 'equation',
      },
      {
      ...
      <div className="row image-input-container">
        ...
        <div className="col-sm-12 col-md-6 order-md-1 user-inputs">
          ...
          <div className="row mb-2 align-items-center">
            <div className="col-4 text-right">Sinh(X):-</div>
            <div className="col-8">
