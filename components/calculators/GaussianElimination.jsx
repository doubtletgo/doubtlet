...
const GaussianElimination = () => {
  const [row, setRow] = useLocalStorage('GaussianElimination_row', '3');
  const [column, setColumn] = useLocalStorage('GaussianElimination_column', '4');
  const [frstMatrix, setFrstMatrix] = useLocalStorage('GaussianElimination_frstMatrix', [
    [2, 3, 1, 9],
    [1, 2, 3, 8],
    [3, 1, 2, 3],
  ]);
  const [equation, setEquation] = useLocalStorage('GaussianElimination_equation', '');
  const [solution, setSolution] = useLocalStorage('GaussianElimination_solution', '');
  const [answer, setAnswer] = useLocalStorage('GaussianElimination_answer', undefined);
  const [showResult, setShowResult] = useLocalStorage('GaussianElimination_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('GaussianElimination_showSteps', true);
  const [note, setNote] = useLocalStorage('GaussianElimination_note', undefined);
