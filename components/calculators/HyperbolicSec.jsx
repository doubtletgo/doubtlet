...
const HyperbolicSec = () => {
  const [latex, setLatex] = useLocalStorage('HyperbolicSec_latex', '\\frac{1}{3}');
  const [a, setA] = useLocalStorage('HyperbolicSec_a', '');
  const [equation, setEquation] = useLocalStorage('HyperbolicSec_equation', '');
  const [solution, setSolution] = useLocalStorage('HyperbolicSec_solution', '');
  const [result, setResult] = useLocalStorage('HyperbolicSec_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('HyperbolicSec_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('HyperbolicSec_showSteps', true);
  const [note, setNote] = useLocalStorage('HyperbolicSec_note', undefined);
  const [input, setInput] = useLocalStorage('HyperbolicSec_input', undefined);
  ...
  useEffect(() => {
    ...
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the value of Sech \\space(\\bold{{${latex
              .replace('\\frac', '')
              .split('}{')
              .join('\\above{1pt}')}}})`
          ),
  ...
  useEffect(() => {
    ...
    const finalAnswer = [
      {
        value: `The \\space value \\space of \\space Sech({${parseNumber(
          p || '(X)'
        )} ${
          q != 1 ? `\\above{1pt} ${parseNumber(q)}}` : `}`
        }) \\space is \\space \\bold{\\frac{2}{(e^{{${parseNumber(p || 'X')}
        ${
          q != 1 ? `\\above{1pt} ${parseNumber(q)}}` : `}`
        }} + e^{-{${parseNumber(p || 'X')}
        ${
    ...
    const steps = [
      ...
      'br',
      {
        value: `The term <b>Sech(X)</b> represents the hyperbolic Sec function.It is a mathematical function <br/> that 
        relates to the hyperbolic trigonometric functions, which are analogues of the standard <br/> trigonometric 
        functions (Sine, Cosine, tangent) in the context of hyperbolic geometry.<br/>
        The hyperbolic Sec function Sech (x), can be defined using exponential functions as follows:
        `,
        type: 'span',
      },
      'br',
      {
        value: `Sech(X) = \\bold{\\frac{2}{(e^X+e^{-X})}}`,
        type: 'equation',
      },
      {
      ...
      'br',
      {
        value: `Sech({${parseNumber(p || 'X')} 
        ${
          q != 1 ? `\\above{1pt} ${parseNumber(q)}}` : `}`
        }) = \\frac{2}{(e^{{${parseNumber(p || 'X')} 
        ${
          q != 1 ? `\\above{1pt} ${parseNumber(q)}}` : `}`
        }} + e^{-{${parseNumber(p || 'X')} 
        ${
      ...
      <div className="row image-input-container">
        ...
        <div className="col-sm-12 col-md-6 order-md-1 user-inputs">
          ...
          <div className="row mb-2 align-items-center">
            <div className="col-4 text-right">Sech(X):-</div>
            <div className="col-8">
