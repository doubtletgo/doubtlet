...
import { convertFromLatex, evalInDecimals } from '@/helpers/matrixHelper';
...
const CubeRoot = () => {
  ...
  const cubeLatex = `\\sqrt[3]{${value}}`;

  const valueInDecimals = evalInDecimals(parsed);
  const hasValue =
    !isInputInvalid(precision) &&
    !isInputInvalid(value) &&
    evalInDecimals(precision) > 0;
  ...
  useEffect(() => {
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `\\LARGE{Find ${cubeLatex} precisely to ${precision} digits after the decimal point.}`
          ),
          type: 'equation',
        },
      ])
  ...
  useEffect(() => {
    setEquation(
      renderSteps([
        ...
        'br',
        {
          value: `Value \\space = \\space ${cubeLatex}`,
          type: 'equation',
        },
      ])
    );
    ...
    const rootValue = Math.cbrt(valueInDecimals);

    const finalValue = rootValue.toPrecision(Number(precision) + 1);

    const finalAnswer = [
      {
        value: putSpace(`\\LARGE{${cubeLatex}  = \\bold{${finalValue}}}`),
        type: 'equation',
      },
    ];
    const equations = [
  ...
  }, [cubeLatex, valueInDecimals, showSteps, precision]);
      ...
      <div className="row image-input-container">
        ...
        <div className="col-sm-12 col-md-6 order-md-1 user-inputs">
          ...
          <div className="row mb-2 align-items-center">
            <div className="col-3 text-left">Precision:</div>
            <div className="col-9">
              <Input
