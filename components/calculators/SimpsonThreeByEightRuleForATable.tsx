  ...
  useEffect(() => {
    ...
    setEquation(
      renderSteps([
        ...
        {
          value: putSpace(
            `Approximate the definite Intergal ${formula}  with the Simpson’s three by eight Rule using the table of points`
          ),
          type: 'equation',
        },
      ])
    ...
    const deltaX = evalExpression(`3*(${sortedX[1]}-(${sortedX[0]}))/8`);
    ...
    const steps = [
      ...
      {
        value: `The Simpson’s three by eight rule uses the points of a subinterval on the parabola for computing the height of the approximating rectangle.`,
        type: 'span',
      },
      'br',
      {
        value: `We can calculate the definite integral by using the Simpson’s 1/8 rule for the given table of points by the formula given below`,
        type: 'span',
      },
      {
      ...
      {
        value: `Where, Height = \\space \\mathit{\\frac{3\\triangle x}{8} = \\frac{3(b-a)}{n}}`,
        type: 'equation',
      },
      'br',
      {
      ...
      {
        value: `Height = \\frac{3(${convertFromLatex(
          sortedX[1]
        )}-(${convertFromLatex(sortedX[0])}))}{8}={${convertIntoLatex(
          deltaX
        )}}`,
        type: 'equation',
      },
      ...
      {
        value: putSpace(
          `Now, just sum up the above obtained values and multiply it by height = \\frac{3\\triangle x}{8} = ${convertIntoLatex(
            deltaX
          )}`
        ),
        type: 'equation',
...
export default SimpsonThreeByEightRuleForATable;
