...
const TanInverse = () => {
  ...
  useEffect(() => {
    ...
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the value of tan^{-1} {${latex
              .replace('\\frac', '')
              .split('}{')
              .join('\\above{1pt}')}}`
          ),
  ...
  useEffect(() => {
    ...
    const radian = parseNumber(Math.atan(input), {}, 3);
    ...
    const finalAnswer = [
      {
        value: putSpace(
          `The principal value of tan^{-1}({${valueToShow}}) is \\bold{${radToDeg(
            radian
      ...
      {
        value: putSpace(
          `\\& General Solution is tan^{-1}({${valueToShow}}) = \\bold{n\\pi + (${isNeg}{${
            p == 0 ? '0' : valueWithPI
          }})}`
        ),
        type: 'equation',
    ...
    const steps = [
      ...
      {
        value:
          putSpace(`The inverse of the tan of the given value is represented as 
        `),
        type: 'equation',
      },
      {
        value: putSpace(
          `y = tan^{-1}(x) or y = arctan(x) or y = atan(x) \\& defined by a function such that x = tan(y)`
        ),
        type: 'equation',
      },
      {
        value: `The domain of the tan inverse x is [-1, 1].`,
        type: 'span',
      },
      {
        value: putSpace(
      ...
      {
        value: putSpace(
          `General Solution: - it will have values from – ∞ to + ∞ so the general solution of`
        ),
        type: 'equation',
      },
      {
        value: putSpace(`this function will be as below.`),
        type: 'equation',
      },
      {
        value: `\\bold{y = (n\\pi +  tan^{-1}(x)) }`,
        type: 'equation',
      },
      {
        value: `where n is an integer & tan<sup>-1</sup>(x) represents the principal values of the function only.`,
        type: 'span',
      },
      {
        value: `<b>Step-1</b>`,
      ...
      },
      {
        value: isNeg
          ? putSpace(
              `Using the following property: tan^{-1}(-x) = -tan^{-1}(x)`
            )
          : '',
      ...
      },
      {
        value: `
          ${
            isNeg ? `tan^{-1}(${valueToShow}) =` : ' '
          } ${isNeg}tan^{-1}({${removeSymbol(valueToShow)}}) = ${isNeg}({${
          p == 0 ? '0' : valueWithPI
      ...
      {
        value: putSpace(
          `Principal Solution is tan^{-1}({${valueToShow}}) = ${isNeg}({${
            p == 0 ? '0' : valueWithPI
      ...
      {
        value: putSpace(
          `General Solution is tan^{-1}({${valueToShow}}) = n\\pi + (${isNeg}{${
            p == 0 ? '0' : valueWithPI
          }})`
        ),
        type: 'equation',
      ...
      {
        value: putSpace(
          `We can take other integral values of n to obtain other values`
        ),
        type: 'equation',
      },
      {
