...
const SolvingSystemOfLinearEquation = () => {
        ...
        },
        {
          value: putSpace(
            `Solve the given system of equations \\begin{Bmatrix} ${eqnMatrix
              ...
              .join('\\\\')}  \\end{Bmatrix} for x, y by using ${method}`
          ),
          type: 'equation',
        },
      ])
    );
  }, [variables, eqnMatrix, resultMatrix, method]);
  ...
  useEffect(() => {
    ...
    if (isInvalid) return;
    let fullArr = eqnMatrix.map((itm, i) => [...itm, resultMatrix[i]]);
    const simpleMatrix = katexArrToSimpleArr(fullArr);
    ...
    const adjointMatrix = transposeOfMatrix(cofactorOfMatrix(eqnMatrix));
    const inverse = multiplyMatrixByValue(
      adjointMatrix,
      determinantOfEqnMatrix
    );
    const xMatrix = multiplyMatrices(inverse, resultMatrix);
      ...
      },
      {
        value: `${variableValues
          .map(
            (itm, i) =>
              `${varArr[i]} = {${itm}}${
                isSame(itm, decimals[i]) ? '' : `=${decimals[i]}`
              }`
          )
          .join(' \\space \\& \\space ')}`,
        type: `equation`,
    ...
    const inverseSteps = [
      {
        value: `We can solve any system of linear equations by Inverse matrix method if
         <br>and only if the determinant of the coefficient matrix should not be equal to zero.`,
        type: 'span',
      },
      {
        value: putSpace(`AX = B then  X = A^{-1}B `),
        type: 'equation',
      },
      {
        value: '<b>Step-1</b>',
      ...
      'br',
      {
        value: `Write the augmented matrix as Ax = B`,
        type: 'span',
      },
      {
        value: `then ${printMatrix(eqnMatrix)} ${printMatrix(
          varArr.slice(0, variables).map((itm) => [itm])
        )}=${printMatrix(resultMatrix)}`,
        type: 'equation',
      },
      {
        value: putSpace(
          `On comparing, we get A =${printMatrix(
            eqnMatrix
          )} and B =${printMatrix(resultMatrix)} `
        ),
        type: 'equation',
      },
      {
        value: putSpace(
          `Now to find the inverse of A = {${printMatrix(
            eqnMatrix
          )}}^{-1} = ${printMatrix(inverse)}`
        ),
        type: 'equation',
      },
      {
        value: `<a href="/calculator/inverse-of-a-matrix/?a=${eqnMatrix}&b=${variables}"  target="_blank">to see the Steps to calculate the inverse, click here</a>`,
        type: 'span',
      },
      'br',
      {
      ...
      'br',
      {
        value: 'Now, the system can be rewritten as ',
        type: 'span',
      },
      {
        value: putSpace(
          ` ${printMatrix(
            varArr.slice(0, variables).map((itm) => [itm])
          )} ={${printMatrix(eqnMatrix)}}^{-1}${printMatrix(
            resultMatrix
          )} = ${printMatrix(inverse)}${printMatrix(
            resultMatrix
          )} = ${printMatrix(simpleArrToKatexArr(xMatrix))}`
        ),
        type: 'equation',
      },
      {
        value: `<a href="/calculator/matrix-multiplication/?a=${inverse}&b=${resultMatrix}&c=${variables}&d=${variables}&e=1"  target="_blank">to see the Steps to multiply, click here</a>`,
        type: 'span',
      },
      'br',
      {
        value: putSpace(
          `After solving, we get ${variableValues
            .map(
              (itm, i) =>
                `${varArr[i]} = {${itm}}${
                  isSame(itm, decimals[i]) ? '' : `=${decimals[i]}`
    ...
    const gaussJordan = [
      {
        value: `We can solve any system of linear equations by Gauss-Jordan Elimination<br>
         method by finding the reduced row echelon form of the augmented matrix.`,
        type: 'span',
      },
      {
        value: putSpace(
          `Write the Augmented matrix as ${printMatrix(
            addVerticalLine(fullArr)
          )}`
        ),
        type: 'equation',
      },
      {
        value: `Now, we will perform the Gauss-Jordan elimination on the above matrix.`,
        type: 'span',
      },
      {
        value: `${printMatrix(addVerticalLine(finalMat))}`,
        type: 'equation',
      },
      {
        value: `<a href="/calculator/gauss-jordan-elimination-calculator/?a=${fullArr}&b=${variables}&c=${
          variables + 1
        }"  target="_blank">to see the Steps to calculate the Gauss Jordan Elimination, click here</a>`,
        type: 'span',
      },
      'br',
      {
        value: putSpace(
          `After back substitution, we get ${variableValues
            .map(
              (itm, i) =>
                `${varArr[i]} = {${itm}}${
                  isSame(itm, decimals[i]) ? '' : `=${decimals[i]}`
    ...
    const stepObj = {
      'Inverse Matrix Method': inverseSteps,
      'Gauss Jordan Elimination': gaussJordan,
      "Cramer's Rule": cramerStep,
    };
    const steps = [
      {
  ...
  }, [showSteps, variables, eqnMatrix, resultMatrix, method]);
  ...
  const clear = useCallback(() => {
    ...
    setResultMatrix([[''], ['']]);
    setMethodType('Inverse Matrix Method');
    setVariables('');
    setShowResult(false);
  }, [setShowResult]);
  const hasValue =
    ...
    <>
      <div className="row image-input-container">
        ...
        <div className="col-sm-12 col-md-6 order-md-1 user-inputs">
          ...
          <div className="dropdown row mb-2 align-items-center">
            <div className="col-4 text-left">Method Type</div>
            <div className="col-8">
              <select
              ...
              >
                <option value="Inverse Matrix Method">
                  Inverse Matrix Method
                </option>
                <option value="Cramer's Rule">{`Cramer's Rule`}</option>
                <option value="Gauss Jordan Elimination">
                  Gauss Jordan Elimination
                </option>
              </select>
            </div>
          </div>
          <div className="col-3 text-left d-flex">Cofficient Matrix: -</div>
          {variables > 0 && (
...
export default SolvingSystemOfLinearEquation;
