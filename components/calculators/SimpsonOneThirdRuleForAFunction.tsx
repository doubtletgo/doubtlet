...
const SimpsonOneThirdRuleForAFunction = () => {
  const [expression, setExpression] = useLocalStorage('SimpsonOneThirdRuleForAFunction_expression', '\\frac{1}{x^2}');
  const [lowerKatex, setLowerKatex] = useLocalStorage('SimpsonOneThirdRuleForAFunction_lowerKatex', '1');
  const [upperKatex, setUpperKatex] = useLocalStorage('SimpsonOneThirdRuleForAFunction_upperKatex', '3');
  const [interval, setSubIntervals] = useLocalStorage('SimpsonOneThirdRuleForAFunction_interval', '4');
  const [equation, setEquation] = useLocalStorage('SimpsonOneThirdRuleForAFunction_equation', '');
  const [solution, setSolution] = useLocalStorage('SimpsonOneThirdRuleForAFunction_solution', '');
  const [answer, setAnswer] = useLocalStorage('SimpsonOneThirdRuleForAFunction_answer', undefined);
  const [invalidInput, setInvalidInput] = useLocalStorage('SimpsonOneThirdRuleForAFunction_invalidInput', false);
  const [showResult, setShowResult] = useLocalStorage('SimpsonOneThirdRuleForAFunction_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('SimpsonOneThirdRuleForAFunction_showSteps', true);
  const [note, setNote] = useLocalStorage('SimpsonOneThirdRuleForAFunction_note', undefined);
