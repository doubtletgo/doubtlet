...
const InverseHyperbolicSine = () => {
  ...
  useEffect(() => {
    ...
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the value of Sinh^{-1}({${latex
              .replace('\\frac', '')
              .split('}{')
              .join('\\above{1pt}')}})`
          ),
  ...
  useEffect(() => {
    ...
    const steps = [
      ...
      'br',
      {
        value: `The term <b>Inverse Sinh(X) or Sinh<sup>-1</sup>(X)</b> represents the Inverse hyperbolic Sine function.
          It is a mathematical function <br/> that relates to the Inverse hyperbolic trigonometric functions, 
          which are analogues of the sSindard inverse hyperbolic trigonometric <br/> functions such as 
          (Inverse Sinh, Inverse Cosh, Inverse Sinh) in the context of Inverse hyperbolic geometry.<br/> 
          It is also commonly denoted as <b>arcSinh(X)</b> or <b>aSinh(X)</b>.</br>
          In words, the inverse Sinh function of a given value X returns the value Y such that Sinh(Y) is equal to X.</br> 
          The Inverse hyperbolic Sine function or Sinh<sup>-1</sup>(X), can be defined using logarithimic 
          functions as follows: `,
        type: 'span',
      },
      'br',
      {
        value: `Sinh^{-1}(X) = \\space \\bold{ln( \\space X+\\sqrt{(1+X^2)} \\space )} \\space where, -inf < x < inf  `,
        type: 'equation',
      },
      'br',
      {
      ...
      'br',
      {
        value: `Sinh^{-1}({${parseNumber(p || 'X')} 
        ${
          q != 1 ? `\\above{1pt} ${parseNumber(q)}}` : `}`
        }) = \\space ln( \\space {{${parseNumber(p || 'X')} 
            ${
              q != 1 ? `\\above{1pt} ${parseNumber(q)}}` : `}`
            }} \\space + \\sqrt{1+({{${parseNumber(p || '(X)')} ${
          q != 1 ? `\\above{1pt} ${parseNumber(q)}}` : `}`
        }})^2}\\space) \\space = \\space \\bold{${math.log(
          p / q + sqrt(1 + (p / q) * (p / q))
        )}} `,
        type: 'equation',
      },
...
export default InverseHyperbolicSine;
