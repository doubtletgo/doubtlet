...
const MagnitudeOfVector = () => {
  const [size, setSize] = useLocalStorage('MagnitudeOfVector_size', '3');
  const [vectorMat, setVectorMat] = useLocalStorage('MagnitudeOfVector_vectorMat', [['3', '4', '4']]);
  const isInvalid = useRef();
  const [equation, setEquation] = useLocalStorage('MagnitudeOfVector_equation', '');
  const [solution, setSolution] = useLocalStorage('MagnitudeOfVector_solution', '');
  const [result, setResult] = useLocalStorage('MagnitudeOfVector_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('MagnitudeOfVector_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('MagnitudeOfVector_showSteps', true);
  const [note, setNote] = useLocalStorage('MagnitudeOfVector_note', undefined);
