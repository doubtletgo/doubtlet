...
const InverseHyperbolicCotangent = () => {
  const [latex, setLatex] = useLocalStorage('InverseHyperbolicCotangent_latex', '\\frac{1}{3}');
  const [a, setA] = useLocalStorage('InverseHyperbolicCotangent_a', '');
  const [equation, setEquation] = useLocalStorage('InverseHyperbolicCotangent_equation', '');
  const [solution, setSolution] = useLocalStorage('InverseHyperbolicCotangent_solution', '');
  const [result, setResult] = useLocalStorage('InverseHyperbolicCotangent_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('InverseHyperbolicCotangent_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('InverseHyperbolicCotangent_showSteps', true);
  const [note, setNote] = useLocalStorage('InverseHyperbolicCotangent_note', undefined);
  const [input, setInput] = useLocalStorage('InverseHyperbolicCotangent_input', undefined);
