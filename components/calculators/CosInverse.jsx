...
const CosInverse = () => {
  ...
  useEffect(() => {
    ...
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the value of Cos^{-1} {${latex
              .replace('\\frac', '')
              .split('}{')
              .join('\\above{1pt}')}}`
          ),
  ...
  useEffect(() => {
    ...
    const radian = parseNumber(Math.acos(input), {}, 3);
    const radian2 = parseNumber(Math.acos(abs(input)), {}, 3);
    const degToRad = fraction(radToDeg(radian), 180);
    const degToRad2 = fraction(radToDeg(radian2), 180);

    setDegree(radToDeg(radian));
    const valueWithPI =
      withSymbol(abs(degToRad[0]), '\\pi') +
      (degToRad[1] == 1 ? '' : '\\above{1pt}' + degToRad[1]);
    const valueWithPI2 =
      withSymbol(abs(degToRad2[0]), '\\pi') +
      (degToRad2[1] == 1 ? '' : '\\above{1pt}' + degToRad2[1]);
    ...
    const finalAnswer = [
      {
        value: putSpace(
          `The principal value of cos^{-1}({${valueToShow}}) is \\bold{${radToDeg(
            radian
          )}^{o}} or \\bold{${valueWithPI}} or \\bold{${radian}radian}.`
        ),
        type: `equation`,
      },
      {
        value: putSpace(
          `\\& General Solution is cos^{-1}({${valueToShow}}) = \\bold{2n\\pi  ±({${valueWithPI}})}`
        ),
        type: 'equation',
      },
    ];
    ...
    const steps = [
      ...
      'br',
      {
        value: `The inverse of the cos of the given value is represented as 
        `,
        type: 'span',
      },
      {
        value: putSpace(
          `y = cos^{-1}(x) or y = arccos(x) or y = acos(x) \\& defined by a function such that x = cos(y)`
        ),
        type: 'equation',
      },
      {
        value: `The domain of the cos inverse x is [-1, 1].`,
        type: 'span',
      },
      {
      ...
      {
        value: `\\bold{y = (2n\\pi ± cos^{-1}(x)) }`,
        type: 'equation',
      },
      {
        value: `where n is an integer & cos<sup>-1</sup>(x) represents the principal values of the function only.`,
        type: 'span',
      },
      'br',
      {
      ...
      },
      {
        value: isNeg
          ? putSpace(
              `Using the following property: cos^{-1}(-x) = \\pi -cos^{-1}(x)`
            )
          : '',
      ...
      {
        value: `
          ${isNeg ? `cos^{-1}(${valueToShow}) =` : ' '} ${
          isNeg ? `\\pi -` : ''
        }cos^{-1}({${removeSymbol(valueToShow)}}) =  ${
          isNeg ? `\\pi - {${valueWithPI2}} =` : ''
        }({${valueWithPI}}) \\space radian \\space or \\space ${radToDeg(
          radian
        )} \\space degree`,
        type: 'equation',
      },
      ...
      {
        value: `
        From the table of the Cos values, we can determine the angle such that`,
        type: 'span',
      },
      {
        value: putSpace(
          `Principal Solution is cos^{-1}({${valueToShow}}) =({${valueWithPI}}) radian or ${radToDeg(
            radian
          )} degrees`
        ),
        type: 'equation',
      ...
      {
        value: putSpace(
          `General Solution is cos^{-1}({${valueToShow}}) = 2n\\pi ± ({${valueWithPI}})`
        ),
        type: 'equation',
      },
      {
