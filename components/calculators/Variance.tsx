...
const Variance = () => {
  const [xValues, setXValues] = useLocalStorage('Variance_xValues', '5.3,11,6,-2,-8');
  const [equation, setEquation] = useLocalStorage('Variance_equation', '');
  const [solution, setSolution] = useLocalStorage('Variance_solution', '');
  const [result, setResult] = useLocalStorage('Variance_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('Variance_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('Variance_showSteps', true);
  const [note, setNote] = useLocalStorage('Variance_note', undefined);
  const [methodType, setMethodType] = useLocalStorage<'Sample' | 'Population'>('Variance_methodType', 'Sample');
      ...
      renderSteps([
        ...
        },
        {
          value: putSpace(
            ...
            } Variance of the given set of values:`
          ),
          type: 'equation',
        },
        {
    ...
    const steps = [
      ...
      'br',
      {
        value: putSpace(
          `The ${
            methodType == 'Sample' ? `\\bold{Sample}` : `\\bold{Population}`
          } variance for the given set of values is given by:`
        ),
        type: 'equation',
      },
      methodType == 'Sample'
        ? {
            value: `\\bold{S^2} = \\frac{\\Sigma^n_{i=1} (x_i - \\mu)^2}{n-1} = \\frac{Sum \\space of(x_i - \\mu)^2}{n-1}`,
            type: 'equation',
          }
        : {
            value: `\\bold{\\sigma^2} = \\frac{\\Sigma^n_{i=1} (x_i - \\mu)^2}{n} = \\frac{Sum \\space of(x_i - \\mu)^2}{n}`,
            type: 'equation',
          },
      {
      ...
      {
        value: `Variance is also called as square of Standard Deviation.`,
        type: 'span',
      },
      'br',
      {
      ...
      methodType == 'Sample'
        ? {
            value: putSpace(
              `So, Sample Variance : \\frac{Sum of (x_i - \\mu_x)^2}{n-1} = \\frac{${evalToDecimals(
                SumOfProducts
        ...
        : {
            value: putSpace(
              `So, Population Variance : \\frac{Sum of (x_i - \\mu_x)}{n} = \\frac{${evalToDecimals(
                SumOfProducts
