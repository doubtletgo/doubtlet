...
const FiveNumberSummary = () => {
  ...
  useEffect(() => {
    setNote(
      renderSteps([
        ...
        {
          value: putSpace(
            `Calculate the Five number summary for given set of values: `
          ),
          type: 'equation',
        },
        {
  ...
  useEffect(() => {
    ...
    const min = statistics.minimum();
    const max = statistics.maximum();
    ...
    const finalAnswer = [
      {
        value: putSpace(`Minimum value = ${convertIntoLatex(min)}`),
        type: 'equation',
      },
      {
        value: putSpace(
          `The Lower (first) Quartile(Q_1) is = ${convertIntoLatex(Q1)}`
        ),
        type: 'equation',
      },
      {
        value: putSpace(
          `The median (second) Quartile(Q_2) is = ${convertIntoLatex(median)}`
        ),
        type: 'equation',
      },
      {
        value: putSpace(
          `The upper (third) Quartile(Q_3) is = ${convertIntoLatex(Q3)}`
        ),
        type: 'equation',
      },
      {
        value: putSpace(`Maximum value = ${convertIntoLatex(max)}`),
        type: 'equation',
      },
    ];
    const equations = [
    ...
    const steps = [
      ...
      {
        value: putSpace(
          `We will calculate Minimum, Q_1, Q_2, Q_3, Maximum for the given set of values.`
        ),
        type: 'equation',
      },
      {
      ...
      'br',
      {
        value: `First, we need to sort the given set of values.`,
        type: 'span',
      },
      {
        value: putSpace(
      ...
      {
        value: putSpace(
          `The Lower (first) Quartile (Q_1) = ${convertIntoLatex(Q1)}`
        ),
        type: 'equation',
      },
      {
      ...
      {
        value: putSpace(
          `The Median (second) Quartile (Q_2) = ${convertIntoLatex(median)}`
        ),
        type: 'equation',
      },
      {
        value: `<a href="/calculator/median-calculator/?a=${values}"  target="_blank">Click here to see steps for median calculation</a>`,
        type: 'span',
      },
      {
        value: putSpace(
          `The Upper (third) Quartile (Q_3) = ${convertIntoLatex(Q3)}`
        ),
        type: 'equation',
      },
      {
...
export default FiveNumberSummary;
