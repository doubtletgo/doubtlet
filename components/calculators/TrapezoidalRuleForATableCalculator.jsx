...
const TrapezoidalRuleForATableCalculator = () => {
  ...
  useEffect(() => {
    ...
    const deltaX = evalExpression(`(${sortedX[1]}-(${sortedX[0]}))/2`);
    const sumOfFrstLast = evalExpression(
      `${sortedF[0]} + (${sortedF[interval - 1]})`
    );
    const sumOfRest = evalExpression(
      `${sortedF
        .slice(1, interval - 1)
        .map((i) => `(${convertFromLatex(i)})`)
        .join(' + ')}`
    );
    const finalResult = evalExpression(
      `${deltaX}*(${sumOfFrstLast} + (2)*(${sumOfRest}))`
    );
    ...
    const finalAnswer = [
      ...
      {
        value: `\\approx ${convertFromLatex(finalResult)} ${decRes}`,
        type: 'equation',
      },
    ];
    ...
    const steps = [
      ...
      },
      {
        value: `Height = \\frac{${convertFromLatex(
          sortedX[1]
        )}-(${convertFromLatex(sortedX[0])})}{2}={${convertIntoLatex(deltaX)}}`,
        type: 'equation',
      },
      ...sortedX.map((x, i) => ({
      ...
      {
        value: putSpace(
          `Now, just sum up the above obtained values and multiply it by height = \\frac{\\triangle x}{2} = ${convertIntoLatex(
            deltaX
          )} = ${evalInDecimals(deltaX)}`
        ),
        type: 'equation',
      },
      {
        value: putSpace(
          `${formula} = (${convertIntoLatex(deltaX)})\\{( ${convertIntoLatex(
            sortedF[0]
          )} + (${convertIntoLatex(sortedF[interval - 1])}) ) + (2)(${sortedF
            .slice(1, interval - 1)
...
export default TrapezoidalRuleForATableCalculator;
