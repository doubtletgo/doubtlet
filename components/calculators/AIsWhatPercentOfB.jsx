...
const AIsWhatPercentOfB = () => {
  const [a, setA] = useLocalStorage('AIsWhatPercentOfB_a', '7');
  const [b, setB] = useLocalStorage('AIsWhatPercentOfB_b', '2');
  const [equation, setEquation] = useLocalStorage('AIsWhatPercentOfB_equation', '');
  const [solution, setSolution] = useLocalStorage('AIsWhatPercentOfB_solution', '');
  const [result, setResult] = useLocalStorage('AIsWhatPercentOfB_result', undefined);
  const [showResult, setShowResult] = useLocalStorage('AIsWhatPercentOfB_showResult', true);
  const [showSteps, setShowSteps] = useLocalStorage('AIsWhatPercentOfB_showSteps', true);
  const [note, setNote] = useLocalStorage('AIsWhatPercentOfB_note', undefined);
  ...
  useEffect(() => {
    setNote(
      renderSteps([
        ...
        {
          value: `Calculate\\space the\\space \\bold{Number} \\space P \\% \\space of \\space \\bold{{${parseNumber(
            r || '1'
          )} ${
  ...
  useEffect(() => {
    ...
    const steps = [
      ...
      'br',
      {
        value: `Let the number be P, then the value of Number P whose % of B is <br>
        A can be obtained by using the formula`,
        type: 'span',
      },
      {
        value: `A = {p \\above{1pt} 100} * B`,
        type: 'equation',
      },
      {
        value: `Then after solving for P`,
        type: 'span',
      },
      {
...
export default AIsWhatPercentOfB;
